function [T,Y, Yplus, Yminus]=multistage_ddm_fpt_dist(a,s,threshold,x0,x0dist,deadlines,tfinal,pWait)
...
% pWait = probability of Wait in each stage [0 0]
%
...
if (length(pWait) < length(stages))
    pWait = [pWait zeros(1, length(stages)-length(pWait))];
end
...
for stage=1:stages-1
    ...
    cdf = cdf.*(1-pWait(stage));
    cdf_p = cdf_p.*(1-pWait(stage));
    cdf_m = cdf_m.*(1-pWait(stage));
    ...
    %% XXX: Want to modify this such that the initial distribution for stage 2 is equal to x0 at pWait proportion?
    x0dist_prev = x0dist;
    [x0, x0dist , ~ , ~, ~, ~] = dist_deadline(a(stage),s(stage),x0, x0dist,t_end,z, 0);
    x0dist = x0dist.*(pWait(stage)) + x0dist_prev.*(1-pWait(stage));
    if length(x0)>1
        step=x0(2)-x0(1);
    else
        step=1;
...
% Final stage
% 2018.01.22: Allow for a one-stage variant
%
if (tfinal>deadlines(stages))
...
% XXX: Aaron - I'm really unsure about this. Why 10 points between 0.0001 and 0.01? Is this a bug - did he mistake the order of params?
step_t = 0.01;
t1     = linspace(0.0001, step_t, 10);
step1  = t1(2)-t1(1);
