...
learnLen = 100;
testLen  = 80;
calibLen = (60 * (subj <= 24)) + (40 * (subj>24));
...
% XXX: Fix this for variable ISI version
...
for calibIdx = 1:length(calibFiles)
    cfd{calibIdx} = load(fullfile(dataDir, calibFiles(calibIdx).name));
...
if (verbose > 0)
    try
        subjSummary = load('subjSummary.mat');
    ...
    % Practice sessions
    numPrac = dir(fullfile(dataDir, ['combray_prac_' ...
                   num2str(subj, '%.2d') ...
                  '*.mat']));
...
%        thisCueProb            = 1 - 0.1 - (maskIdx/10); % 0.8, 0.7, 0.6, 0.5
    ...
    if (exist('subjSummary.mat', 'file'))
        save('subjSummary.mat', 'subjSummary', '-append');
    else
        save('subjSummary.mat', 'subjSummary');
    end
end
